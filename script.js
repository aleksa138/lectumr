...
function showApp(email) {
    ...
    if (user && user.avatar) {
        avatarEl.innerHTML = `<img src="${user.avatar}" class="avatar-img" alt="Аватар">`;
        profilePreview.innerHTML = `<img src="${user.avatar}" class="avatar-img" alt="Аватар">`;
    } else {
        avatarEl.innerHTML = '👤';
...
function renderSubjects() {
    ...
    if (subjects.length === 0) {
        list.innerHTML = '<div class="placeholder-text">Нет предметов</div>';
        return;
    }
...
function renderFiles(files) {
    ...
    files.forEach((file) => {
        ...
        const shareButton = !file.importedFrom
            ? `<button class="btn-sm" onclick="shareFile(event, '${file.id}')" title="Поделиться" aria-label="Поделиться">🔗</button>`
            : '';
        ...
        div.innerHTML = `
            ...
            <div class="file-actions">
                ${shareButton}
                <button class="btn btn-danger" style="padding:4px 8px; font-size:0.7rem;" onclick="deleteFile(event, '${file.id}')" title="Удалить" aria-label="Удалить">🗑️</button>
            </div>
        `;

        div.querySelector('.file-left').onclick = () => previewFile(file);
        div.querySelector('.file-left').onkeydown = (e) => {
            if (e.key === 'Enter' || e.key === ' ') {
                previewFile(file);
            }
        };
...
function previewFile(file) {
    ...
    if (file.type.includes('pdf')) {
        ...
        box.innerHTML = `<img src="${file.blobUrl}" style="max-width:100%; max-height:100%; object-fit: contain;" alt="${file.name}">`;
    } else {
        box.innerHTML = `<div style="color:white; padding: 20px;">Предпросмотр недоступен</div>`;
    }
}
...
function shareFile(event, fileId) {
    ...
    if (!subject || !subject.files) {
        alert('⚠️ Предмет или файлы не найдены');
        return;
    }
    ...
    if (file.importedFrom) {
        alert('⚠️ Нельзя поделиться импортированным файлом');
        return;
    }
